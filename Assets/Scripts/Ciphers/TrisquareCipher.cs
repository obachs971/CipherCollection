...
public class TrisquareCipher 
{
	public PageInfo[] encrypt(string word, string id, string log, KMBombInfo Bomb)
	{
		...
		Debug.LogFormat("{0} [Trisquare Cipher] Screen 1: {1}", log, replaceJ);
		...
		Debug.LogFormat("{0} [Trisquare Cipher] Screen 2: {1}", log, intersection);
		ScreenInfo[] screens = new ScreenInfo[9];
		screens[0] = new ScreenInfo(replaceJ, new int[] { 35, 35, 35, 32, 28 }[replaceJ.Length - 4]);
		screens[1] = new ScreenInfo(intersection, new int[] { 25, 25, 20 }[intersection.Length - 2]);
		...
		screens[6] = new ScreenInfo(kws[2], new int[] { 35, 35, 35, 32, 28 }[kws[2].Length - 4]);
		screens[7] = new ScreenInfo(kwFronts[2][0], 25);
