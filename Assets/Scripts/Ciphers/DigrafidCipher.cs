...
public class DigrafidCipher
{
	public PageInfo[] encrypt(string word, string id, string log, KMBombInfo Bomb)
	{
		...
		for(int i = 0; i < kws.Length; i++)
		{
			...
			kws[i] = words[len][UnityEngine.Random.Range(0, words[len].Count)].ToUpperInvariant();
			words[len].Remove(kws[i]);
			kwFront[i] = cm.generateBoolExp(Bomb);
			keys[i] = cm.getKey(kws[i], "ABCDEFGHIJKLMNOPQRSTUVWXYZ", kwFront[i][1][0] == 'T');
		}
		string encrypt, letters = choices[0][UnityEngine.Random.Range(0, choices[0].Length)] + "", key = "123456789";
		...
		while (true)
		{
			encrypt = "";
			nums = new string[3] { "", "", "" };
			letters = letters[0] + "" + choices[1][UnityEngine.Random.Range(0, choices[1].Length)];
			...
			if(choices[1].Length == 0)
			{
				...
				letters = choices[0][UnityEngine.Random.Range(0, choices[0].Length)] + "";
				choices[0] = choices[0].Replace(letters, "");
			}
		}
		if (word.Length % 2 == 1)
