...
public class CondiCipher 
{
	public PageInfo[] encrypt(string word, string id, string log, KMBombInfo Bomb)
	{
		...
		Debug.LogFormat("{0} [Condi Cipher] Key Front Rule: {1} -> {2}", log, keyFront[0], keyFront[1]);
		Debug.LogFormat("{0} [Condi Cipher] Key: {1}", log, key);
		Debug.LogFormat("{0} [Condi Cipher] Starting Offset: {1} -> {2}", log, letter, offset);
		Debug.LogFormat("{0} [Condi Cipher] Invert Rule: {1} -> {2}", log, invert[0], invert[1]);
		if (invert[1][0] == 'T')
		{
